//
//  ReadyDetailViewController.swift
//  Mercury App IOS
//
...
class ReadyDetailViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    @IBOutlet weak var readyDetailTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        self.title = "Ready Order"
        
        readyDetailTableView.dataSource = self
        readyDetailTableView.delegate = self
        
        let nibName0 = UINib(nibName: "ReadyContactTableViewCell", bundle: nil)
        readyDetailTableView.register(nibName0, forCellReuseIdentifier: "ready_contact_cell")
        
        let nibName = UINib(nibName: "ReadyDetailTableViewCell", bundle: nil)
        readyDetailTableView.register(nibName, forCellReuseIdentifier: "ready_detail_cell")
    ...
    func loadData() {
        ...
        let predicate1 = NSPredicate(format: "order_status == %@", "Ready")
        ...
        do {
            ...
            initialLblData.removeAll()
            customerLblData.removeAll()
            customerMobileData.removeAll()
            customerEmailData.removeAll()
            
            manufacturerDateLblData.removeAll()
            if results.count > 0 {
        ...
        if rowCounter > 0 {
            readyDetailTableView.reloadData()
        }
        else {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            let cell = readyDetailTableView.dequeueReusableCell(withIdentifier: "ready_contact_cell", for: indexPath) as! ReadyContactTableViewCell
            ...
            let cell = readyDetailTableView.dequeueReusableCell(withIdentifier: "ready_detail_cell", for: indexPath) as! ReadyDetailTableViewCell
            
            let strManufacturer:String = manufacturerLblData.count > 0 ? manufacturerLblData[indexPath.row - 1] : ""
    ...
    func pop_action(_ sender: UIButton) {
        ...
        actionSheet.addAction(UIAlertAction(title: "Mark as Delivered", style: .default, handler: { (action: UIAlertAction) in
                ...
                if results.count > 0 {
                    for result in results as! [NSManagedObject] {
                        ...
                        if let _:String = result.value(forKey: "order_status") as? String {
                            result.setValue("Delivered", forKey: "order_status")
                        }
