//
//  DeliveredDetailTableViewCell.swift
//  Mercury App IOS
//
...
class DeliveredDetailTableViewCell: UITableViewCell {
    ...
    func setOrderViewLayout(itemCount: String, prevImage: UIImage, desc: String, titleArray: [String], detailArray: [String], orderSubIdArray: String, counter: Int, txt_manufacturer: String, txt_mobile: String, txt_email: String, orderDate: String, deliveryDate: String, orderDateData: Date, deliveryDateData: Date) {
...
//        vBtn_view.addSubview(btn_menu)
        ...
        let lbl_processInfo = UILabel(frame: CGRect(x: 0, y: 0, width: vLayout.frame.width, height: 30))
        let formatter = DateFormatter()
        formatter.dateFormat = "dd/MM/yyyy HH:mm"
        
        let strProcessDate = formatter.string(from: orderDateData)
        lbl_processInfo.text = "Accepted & In Process: " + strProcessDate
        vLayout.addSubview(lbl_processInfo)
        
        let lbl_readyInfo = UILabel(frame: CGRect(x: 0, y: 0, width: vLayout.frame.width, height: 30))
        
        let strReadyDate = formatter.string(from: deliveryDateData)
        lbl_readyInfo.text = "Ready on: " + strReadyDate
        vLayout.addSubview(lbl_readyInfo)
        
        let lbl_deliveredInfo = UILabel(frame: CGRect(x: 0, y: 0, width: vLayout.frame.width, height: 30))
        
        let strDeliveredDate = formatter.string(from: deliveryDateData)
        lbl_deliveredInfo.text = "Delivered on: " + strDeliveredDate
        vLayout.addSubview(lbl_deliveredInfo)
    }
